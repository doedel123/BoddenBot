...
export async function claudeCreate(
  params: Parameters<typeof claude.messages.create>[0]
): Promise<ReturnType<typeof claude.messages.create> extends Promise<infer T> ? T : never> {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return await claude.messages.create({ ...params, model: CLAUDE_MODEL_PRIMARY }) as any;
  } catch (err: unknown) {
    ...
    if (status === 529) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return await claude.messages.create({ ...params, model: CLAUDE_MODEL_FALLBACK }) as any;
    }
    throw err;
  }
}
...
export function claudeStream(
  ...
  return claude.messages.stream({ ...params, model: CLAUDE_MODEL });
}
